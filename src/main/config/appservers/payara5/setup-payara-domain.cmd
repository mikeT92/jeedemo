rem setup-payara-domain.cmd
rem --------------------------------------------------------------------------
rem Creates a new payara server domain (i.e. instance) in the specified 
...
set PAYARA_DOMAIN_ROOT=z:\data\payara\domains
set PAYARA_DEFAULT_DOMAIN_NAME=PYFP50_JEEDEMO_DEV
set PAYARA_DOMAIN_NAME=%1

rem use default domain name if no one is given
if not defined PAYARA_DOMAIN_NAME (
	set PAYARA_DOMAIN_NAME=%PAYARA_DEFAULT_DOMAIN_NAME%
)
 
set PAYARA_DOMAIN_PATH=%PAYARA_DOMAIN_ROOT%\%PAYARA_DOMAIN_NAME%

rem check if domain exists
if not exist "%PAYARA_DOMAIN_PATH%" goto create
echo "payara domain %PAYARA_DOMAIN_PATH% already exists"
set /p choice="delete? (y/n)"
if "%choice%" == "y" rmdir /s /q %PAYARA_DOMAIN_PATH%

:create
call asadmin create-domain --domaindir %PAYARA_DOMAIN_ROOT% --portbase 8000 --checkports true --nopassword %PAYARA_DOMAIN_NAME%

rem copy drivers etc to payara lib folder
copy z:\tools\lib\jdbc\postgres*.jar %PAYARA_DOMAIN_PATH%\lib

rem start payara domain to setup server instance
call asadmin start-domain --domaindir %PAYARA_DOMAIN_ROOT% %PAYARA_DOMAIN_NAME%
...
call asadmin --port 8048 set resources.jdbc-connection-pool.JEEDEMO_POOL.slow-query-threshold-in-seconds=15
...
call asadmin --port 8048 stop-domain --domaindir %PAYARA_DOMAIN_ROOT% %PAYARA_DOMAIN_NAME%
endlocal